...
def run_original():
  path_test = f'test.txt'
  path_train_original = f'train.txt'
...
def run(dataset_name,aug_method,n_sample):
    ...
    path_train_aug = f'train_{dataset_name}_{aug_method}_n_sample_{n_sample}.csv'
    path_test = f'test.txt'
...
loss_org, accuracy_org, precision_org, recall_org, auc_org, f1_score_org = run_original()
...
for n in n_samples:
  loss, accuracy, precision, recall, auc, f1_score = run('pc','wordnet_augmenter',n)
...
df_compare.to_csv('pc_wordnet_compare_with_small_w2v.csv')