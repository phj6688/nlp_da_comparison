...
def aug_samples(dataset_name, aug_method, n_sample,org=True):
    path = f'data/{dataset_name}/train.txt'
    data = load_data(path)
    ...
    augmented_data.to_csv(f'data/{dataset_name}/train_{aug_method}_{n_sample}.csv',index=False)

    np.savetxt(f'data/{dataset_name}/train_{aug_method}_{n_sample}.txt', augmented_data.values, fmt='%s', delimiter='\t')
    print(f'{dataset_name} {aug_method} {n_sample} done')
